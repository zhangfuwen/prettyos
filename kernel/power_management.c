...
static bool no_action(PM_STATES state)
{
	switch(state)
	{
...
#define APM_CHECK    ((void*)0x51A)
#define APM_SETSTATE ((void*)0x53A)

bool apm_install()
{
    memcpy((void*)0x500, &apm_com_start, (uintptr_t)&apm_com_end - (uintptr_t)&apm_com_start);
...
void pm_install()
{
	...
	pm_systems[PM_NO].action = &no_action;
