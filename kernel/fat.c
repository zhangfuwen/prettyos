...
// global error
/// #define _GLOBAL_FS_ERROR_

#ifdef _GLOBAL_FS_ERROR_
uint8_t  FSerrno;
#endif
...
static FILEROOTDIRECTORYENTRY cacheFileEntry(FILE* fileptr, uint32_t* curEntry, bool ForceRead)
{
    ...
    textColor(0x0C);
    printf("UNCLEAN END of cacheFileEntry!");
    textColor(0x0F);
}
...
FS_ERROR fileErase( FILE* fileptr, uint32_t* fHandle, bool EraseClusters)
{
    ...
    else
    {
        ...
        if ( ((writeFileEntry( fileptr, fHandle)) == false) || (status != CE_GOOD) )
        {
...
FILE* fopen(const char* fileName, const char* mode)
{
    ...
    FILE* filePtrTemp = (FILE*) malloc(sizeof(FILE),PAGESIZE); // why?
